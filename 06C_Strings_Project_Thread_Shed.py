...
for element_outer in daily_transactions_split:
  transactions_clean_inner = []
  for point_inner in element_outer:
    transactions_clean_inner.append(point_inner.replace("\n", "").strip(" "))
  transactions_clean.append(transactions_clean_inner)
...
for single_TC in transactions_clean:
  customers.append(single_TC[0])
  sales.append(single_TC[1])
  thread_sold.append(single_TC[2])
...
for single_sale in sales:
  sales_interim.append(single_sale.strip('$'))
...
for single_sale in sales_interim:
  sales_interim_float.append(float(single_sale))

print(sales_interim_float)

for single_sif in sales_interim_float:
  total_sales += single_sif
...
thread_sold_split = []
thread_sold_split_interim = []

for single_thread in thread_sold:
  if (single_thread.find('&') == -1):
    thread_sold_split.append(single_thread)
  elif (single_thread.find('&') != -1):
    thread_sold_split_interim.append(single_thread.split('&'))

print("thread_sold_split_interim: ")
print(thread_sold_split_interim)

thread_sold_split_interim_2 = []

for single_interim in thread_sold_split_interim:
  for small in range(0,len(single_interim)):
    #print(single_interim[small])
    thread_sold_split_interim_2.append(single_interim[small])
    thread_sold_split.append(single_interim[small])

#print("thread_sold_split_interim: ")
#print(thread_sold_split_interim)

#print("thread_sold_split_interim_2: ")
#print(thread_sold_split_interim_2)
'''
task17:
...
def f_color_count(color):
  color_count = 0
  #f_color_count_bool_checker = (single_color == color)
  for single_color in thread_sold_split:
    if (single_color == color):
      #print(single_color)
      color_count += 1
  return color_count

print("The count of a given color is: " +str(f_color_count("white")))
...
print("Here is the statistic of threads sold today:")
for i in range(0,len(colors)):
  print(str(colors[i]) + " was sold {} times".format(f_color_count(colors[i])))
